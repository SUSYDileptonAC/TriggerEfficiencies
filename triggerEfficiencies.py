...
def studyTriggerBias(path,source,plots,selection,runRange,backgrounds):			
	for name in plots:
		...
		treesDenominatorEE = readTrees(path,"EE",source = "Summer12", modifier = "%s"%("TriggerEfficiency",))
		treesDenominatorMuMu = readTrees(path,"MuMu",source = "Summer12", modifier = "%s"%("TriggerEfficiency",))
		treesDenominatorEMu = readTrees(path,"EMu",source = "Summer12", modifier = "%s"%("TriggerEfficiency",))
		
		
		treesNominatorEE = readTrees(path,"EE",source = "Summer12",modifier="%sHLTDiEle%s"%("TriggerEfficiency",""))
		treesNominatorMuMu = readTrees(path,"MuMu",source = "Summer12",modifier="%sHLTDiMu%s"%("TriggerEfficiency",""))
		treesNominatorMuMuNoTrack = readTrees(path,"MuMu",source = "Summer12",modifier="%sHLTDiMuNoTrackerMuon%s"%("TriggerEfficiency",""))
		treesNominatorEMu = readTrees(path,"EMu",source = "Summer12",modifier="%sHLTEleMu%s"%("TriggerEfficiency",""))
		treesNominatorMuE = readTrees(path,"EMu",source = "Summer12",modifier="%sHLTMuEle%s"%("TriggerEfficiency",""))
		treesNominatorMuEG = readTrees(path,"EMu",source = "Summer12",modifier="%sHLTMuEG%s"%("TriggerEfficiency",""))
		
		eventCounts = totalNumberOfGeneratedEvents(path,"Summer12")	
		processes = []
		for background in backgrounds:
			processes.append(Process(getattr(Backgrounds,background),eventCounts))
		...
		denominatorHistoEENoTrig = denominatorStackEE.theHistogram
		denominatorHistoMuMuNoTrig = denominatorStackMuMu.theHistogram
		denominatorHistoMuMuNoTrackNoTrig = denominatorStackMuMu.theHistogram
		denominatorHistoEMuNoTrig = denominatorStackEMu.theHistogram
		denominatorHistoMuENoTrig = denominatorStackMuE.theHistogram
		denominatorHistoMuEGNoTrig = denominatorStackMuEG.theHistogram
		
		nominatorHistoEENoTrig = nominatorStackEE.theHistogram
		nominatorHistoMuMuNoTrig = nominatorStackMuMu.theHistogram
		nominatorHistoMuMuNoTrackNoTrig = nominatorStackMuMuNoTrack.theHistogram
		nominatorHistoEMuNoTrig = nominatorStackEMu.theHistogram
		nominatorHistoMuENoTrig = nominatorStackMuE.theHistogram
		nominatorHistoMuEGNoTrig = nominatorStackMuEG.theHistogram		
